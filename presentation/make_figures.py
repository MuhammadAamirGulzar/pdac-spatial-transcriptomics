...
# ============================================================ FIG 3 - leaving maps + tissue
def fig3_leaving_maps():
    moran = {"IU_PDA_T1": 0.39, "IU_PDA_T3": 0.61, "IU_PDA_T4": 0.37, "IU_PDA_T11": 0.37}
    if SLIDE:
        fig, axs = plt.subplots(2, 4, figsize=(18, 9.2)); fig.subplots_adjust(top=0.88, bottom=0.04)
        pos = {"IU_PDA_T1": (0, 0), "IU_PDA_T3": (0, 2), "IU_PDA_T4": (1, 0), "IU_PDA_T11": (1, 2)}
        for s in PT:
            r, c = pos[s]; d = lead[lead["sample"] == s]
            he_panel(axs[r, c], s); axs[r, c].set_title(f"{short(s)}  H&E", fontsize=12, fontweight="bold")
            sc = score_panel(axs[r, c + 1], s, d["row"], d["col"], d["leaving_score"].values, DIVCMAP, -2.5, 2.5, s=7)
            axs[r, c + 1].set_title(f"{short(s)}  leaving score  (I={moran[s]:.2f})", fontsize=12, fontweight="bold")
            fig.colorbar(sc, ax=axs[r, c + 1], fraction=0.046, pad=0.02)
        fig.suptitle("The leaving program forms coherent zones in every tumour (red = high)",
                     fontsize=16, fontweight="bold", y=0.97)
    else:
        fig, axs = plt.subplots(4, 2, figsize=(11, 18))
        for i, s in enumerate(PT):
            d = lead[lead["sample"] == s]
            he_panel(axs[i, 0], s); axs[i, 0].set_title(f"{short(s)}  H&E tissue", fontsize=12, fontweight="bold")
            sc = score_panel(axs[i, 1], s, d["row"], d["col"], d["leaving_score"].values, DIVCMAP, -2.5, 2.5, s=11)
        ...
        caption(fig, "Right: each spot coloured by its EMT/invasion leaving-program score (red = high). Left: the same tissue in H&E. "
                     "High-score regions form coherent zones (positive, significant Moran's I in every slide), consistent with invasive fronts.", y=0.004)
...
def fig6_spatial_pred():
    samples = ["IU_PDA_T3", "IU_PDA_T1"]
    fig, axs = plt.subplots(len(samples), 3, figsize=(15, 5.3*len(samples)))
    for i, s in enumerate(samples):
        d = pb[pb["sample"] == s]
        he_panel(axs[i, 0], s); axs[i, 0].set_title(f"{short(s)} — H&E", fontsize=12, fontweight="bold")
        sc1 = score_panel(axs[i, 1], s, d["row"], d["col"], d["leaving_score"].values, DIVCMAP, -2.5, 2.5, s=11)
        ...
        sc2 = score_panel(axs[i, 2], s, d["row"], d["col"], d["pred_raw_smooth"].values,
                          DIVCMAP, np.percentile(d["pred_raw_smooth"],2), np.percentile(d["pred_raw_smooth"],98), s=11)
    ...
    fig.suptitle("Predicted from H&E versus the real leaving score, shown on the tissue",
                 fontsize=15.5, fontweight="bold")
