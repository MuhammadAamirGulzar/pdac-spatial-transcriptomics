...
def header(s, title, page):
    ...
    txt(s, f"{page:02d}", 16.7, 0.0, 0.9, 0.83, 16, LIGHT, anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER)
    footer(s)
...
fig_slide("Why we do not simply compare the two tissue types", 6, os.path.join(SL, "fig2_stage0_confound.png"),
          "Liver metastasis tissue is full of liver cells, and the two metastasis patients do not resemble each other.  So we look inside the primary tumour instead.",
          "The obvious idea is to compare primary tumour to liver metastasis and call the difference metastasis. "
          "That difference is mostly liver. Metastasis spots are about fifteen percent liver cells, primary spots almost none. "
          "And with only two metastasis patients, one does not predict the other. So instead of comparing tissue types we "
          "define a movement program inside the primary tumour, which avoids the liver confound entirely.")
...
fig_slide("Reading one result end to end", 9, os.path.join(FIG, "fig_worked_example.png"),
...
fig_slide("What an H&E slide can and cannot tell us", 11, os.path.join(SL, "fig5_stages23_ceiling.png"),
...
fig_slide("The same result, drawn on the tissue", 12, os.path.join(SL, "fig6_stage3_spatial_prediction.png"),
...
fig_slide("Scorecard and honest verdict", 14, os.path.join(SL, "fig7_stage4_scorecard.png"),
...
out = os.path.join(PROJ, "docs", "meetings", "Meeting - Results and Findings (PDAC ST + Foundation Model).pptx")
prs.save(out)
